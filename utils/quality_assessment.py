...
def _reshape_input_torch(
    tensor: torch.Tensor,
) -> Tuple[torch.Tensor, Optional[int], Optional[int], int, int]:
    if tensor.dim() == 4:
        b, c, h, w = tensor.size()
    elif tensor.dim() == 3:
        c, h, w = tensor.size()
...
def _reshape_output_torch(
    tensor: torch.Tensor, b: Optional[int], c: Optional[int]
) -> torch.Tensor:
    rh = tensor.size(-2)
    rw = tensor.size(-1)
