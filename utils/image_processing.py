...
def _cubic(x: float) -> float:
    ...
    cubic2 = (-0.5 * abs_x3 + 2.5 * abs_x2 - 4 * abs_x + 2) * (
...
def _mesh_grid(kernel_size: int) -> List[np.ndarray]:
...
def _calculate_rotate_sigma_matrix(
...
def _calculate_probability_density(
    sigma_matrix: np.ndarray, grid: np.ndarray
) -> np.ndarray:
...
def _calculate_cumulative_density(
    skew_matrix: np.ndarray, grid: np.ndarray
) -> np.ndarray:
...
def _random_bivariate_gaussian_kernel(
    kernel_size: int,
    sigma_x_range: tuple,
    sigma_y_range: tuple,
...
def _random_bivariate_generalized_gaussian_kernel(
    kernel_size: int,
    sigma_x_range: tuple,
    sigma_y_range: tuple,
    rotation_range: tuple,
...
def _random_bivariate_plateau_gaussian_kernel(
    kernel_size: int,
    sigma_x_range: tuple,
    sigma_y_range: tuple,
    rotation_range: tuple,
...
def random_mixed_kernels(
    ...
    if kernel_type == "isotropic":
        mixed_kernels = _random_bivariate_gaussian_kernel(
            kernel_size, sigma_x_range, sigma_y_range, rotation_range, noise_range, True
        )
        ...
        mixed_kernels = _random_bivariate_generalized_gaussian_kernel(
            kernel_size,
            sigma_x_range,
            sigma_y_range,
            rotation_range,
        ...
        mixed_kernels = _random_bivariate_plateau_gaussian_kernel(
            kernel_size,
            sigma_x_range,
            sigma_y_range,
            rotation_range,
...
def generate_sinc_kernel(
    cutoff: float, kernel_size: int, padding: int = 0
) -> np.ndarray:
    """2D sinc filter
    ...
    sinc_kernel[(kernel_size - 1) // 2, (kernel_size - 1) // 2] = cutoff**2 / (
        4 * np.pi
    )
...
def _generate_gaussian_noise(
    image: np.ndarray, sigma: float = 10.0, gray_noise: bool = False
) -> np.ndarray:
...
def _generate_poisson_noise(
    image: np.ndarray, scale: float = 1.0, gray_noise: bool = False
) -> np.ndarray:
...
def _random_generate_gaussian_noise(
    image: np.ndarray, sigma_range: tuple = (0, 10), gray_prob: int = 0
) -> np.ndarray:
...
def _random_generate_poisson_noise(
    image: np.ndarray, scale_range: tuple = (0, 1.0), gray_prob: int = 0
) -> np.ndarray:
...
def _random_add_gaussian_noise(
    image: np.ndarray,
...
def _generate_gaussian_noise_torch(
    image: torch.Tensor, sigma: float = 10.0, gray_noise: torch.Tensor = 0
) -> torch.Tensor:
...
def _generate_poisson_noise_torch(
    image: torch.Tensor, scale: float = 1.0, gray_noise: torch.Tensor = 0
) -> torch.Tensor:
...
def _random_generate_gaussian_noise_torch(
    image: torch.Tensor, sigma_range: tuple = (0, 10), gray_prob: torch.Tensor = 0
) -> torch.Tensor:
...
def _random_generate_poisson_noise_torch(
    image: torch.Tensor, scale_range: tuple = (0, 1.0), gray_prob: torch.Tensor = 0
) -> torch.Tensor:
...
def random_add_gaussian_noise_torch(
    image: torch.Tensor,
    sigma_range: tuple = (0, 1.0),
    gray_prob: int = 0,
    clip: bool = True,
...
def random_add_poisson_noise_torch(
    image: torch.Tensor,
    scale_range: tuple = (0, 1.0),
    gray_prob: int = 0,
    clip: bool = True,
...
def _add_jpeg_compression(image: np.ndarray, quality: int) -> np.ndarray:
    ...
    _, encode_image = cv2.imencode(".jpg", image * 255.0, encode_params)
    jpeg_image = np.float32(cv2.imdecode(encode_image, 1)) / 255.0
...
class _ChromaSubsampling(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        cb = F.avg_pool2d(
            image[:, 1, :, :].unsqueeze(1), (2, 2), (2, 2), count_include_pad=False
        )
        cr = F.avg_pool2d(
            image[:, 2, :, :].unsqueeze(1), (2, 2), (2, 2), count_include_pad=False
        )
...
class _YQuantize(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, factor: int or float or torch.Tensor
    ) -> torch.Tensor:
...
class _CompressJPEG(nn.Module):
    ...
    def forward(self, x: torch.Tensor, factor: torch.Tensor) -> List[torch.Tensor]:
...
class _DeCompressJPEG(nn.Module):
    ...
    def forward(
        ...
        out = self.de_chroma_subsampling(
            components["y"], components["cb"], components["cr"]
        )
        out = self.ycbcr_to_rgb(out)

        out = torch.min(
            255 * torch.ones_like(out), torch.max(torch.zeros_like(out), out)
        )
...
class DiffJPEG(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, quality: int or float or torch.Tensor
    ) -> torch.Tensor:
        factor = quality
        if isinstance(factor, (int, float)):
...
def _usm_sharp(
    image: np.ndarray, weight: float, radius: int, threshold: int
) -> np.ndarray:
    if radius % 2 == 0:
        radius += 1
...
def image_resize(
    image: np.ndarray or torch.Tensor, scale_factor: float, antialiasing: bool = True
...
def random_rotate(
    image, angles: list, center: tuple = None, scale_factor: float = 1.0
) -> np.ndarray:
